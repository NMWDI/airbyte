...
class HydroVuStream(HttpStream, ABC):
    ...
    `class HydrovuJirStream(HttpStream, ABC)` which is the current class
    `class Customers(HydrovuJirStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(HydrovuJirStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalHydrovuJirStream((HydrovuJirStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
...
# Source
class SourceHydrovuJir(AbstractSource):
