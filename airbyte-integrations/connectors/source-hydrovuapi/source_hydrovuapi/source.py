...
class Parameters(HydroVuStream):
    ...
    def path(
        ...
        """
        End of URL path to friendly names from API
        """
    ...
    def request_headers(
        ...
        # If there is a next_page_token from the previous page of friendly names,
...
class Units(HydroVuStream):
    """
    Gets units from friendly names from HydroVu API
    """
...
class SourceHydrovuapi(AbstractSource):
    ...
    def streams(self, config: Mapping[str, Any]) -> List[Stream]:
        ...
        return [Parameters(authenticator=auth), Units(authenticator=auth), Locations(authenticator=auth), Readings(auth, authenticator=auth)]
